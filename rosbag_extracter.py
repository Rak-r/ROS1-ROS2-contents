...
class RosbagExtracter(Node):

    def __init__(self):
        super().__init__('rosbag_extracter')
...
def main(args=None):
    ...
    rosbag_extracter = RosbagExtracter()
    while rclpy.ok() and not rosbag_extracter.data_process:
        rclpy.spin_once(rosbag_extracter)
    rosbag_extracter.plot_data()
    rosbag_extracter.destroy_node()
    rclpy.shutdown()
